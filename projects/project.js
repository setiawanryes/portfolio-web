...
// ============================
// Nested replies + modal reply
// ============================

let replyTo = null; // menyimpan komentar yang sedang dibalas

// Event delegation: klik tombol Reply
komentarList.addEventListener("click", e => {
  ...
  // tombol Reply
  if (target.classList.contains("cmtApp-reply")) {
    replyTo = comment; // simpan comment yang akan dibalas
    document.getElementById("cmtAppModalTitle").innerText = "Balas Komentar";
...
kirimBtn.addEventListener("click", async () => {
  ...
  try {
    if (replyTo) {
      // Simpan reply ke subcollection 'replies' dari komentar yang dibalas
...
// Fungsi rekursif untuk render komentar + nested replies
function renderComment(docSnap, container) {
  const data = docSnap.data();
  const id = docSnap.id;

  const commentEl = document.createElement("div");
  commentEl.classList.add("cmtApp-comment");
  ...
  // Nested replies realtime
  const repliesRef = collection(db, "comments", id, "replies");
  onSnapshot(query(repliesRef, orderBy("timestamp", "asc")), snapshot => {
    const repliesContainer = commentEl.querySelector(".cmtApp-replies");
